...
## 지시 구체성 수준 (Degrees of Freedom)

작업의 취약성(fragility)에 비례하여 지시의 구체성을 조절하세요. 좁은 다리에는 가드레일, 열린 들판에는 자유.

| 수준 | 언제 | 형식 | 예시 |
|------|------|------|------|
| **High** (자유) | 여러 접근법이 유효할 때 | 텍스트 지침 | "사용자 친화적으로 에러를 처리하세요" |
| **Medium** (가이드) | 선호 패턴이 있을 때 | 의사코드, 파라미터 | "retry 3회, backoff 2x, 최대 30초" |
| **Low** (고정) | 일관성이 핵심일 때 | 구체적 코드/스크립트 | 정확한 코드 블록, 커맨드 |

**판단 질문:** "이 부분이 잘못되면 얼마나 치명적인가?"
- 치명적 → Low freedom (정확한 형식 강제)
- 미미 → High freedom (원칙만 제시)
